document.addEventListener("DOMContentLoaded", () => {
    ...
    function aplicarImagensCartas(pasta) {
        ...
        const nomes = ["zero", "um", "dois", "tres", "quatro", "cinco", "seis", "sete"];

        cards.forEach(card => {
            const tipo = card.getAttribute("data-framework");
            const frontFace = card.querySelector(".front-face");
            const backFace = card.querySelector(".back-face");

            if (nomes.includes(tipo)) {
                frontFace.src = `imagens/temas/${pasta}/${tipo}.png`;
            }

            backFace.src = `imagens/temas/${pasta}/verso.png`;
        });
    }
